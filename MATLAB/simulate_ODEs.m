...
% dMM_i/dt = L_i+1 - L_i + L_fi - Qi/lambda
    ...
    for n = 1:p.N-1
        if n == 2
            ddt_MM(n) = u.LF(t) + v.L(n+1,:) - v.L(n,:);  % mol/min, Added feed flowrate to MM2
        else
            ddt_MM(n) = v.L(n+1,:) - v.L(n,:);            % mol/min, Molar holdup - MM1:MM3
        end
    end
    ddt_MM(p.N) = v.LR - v.L(p.N,:);                    % mol/min, Molar holdup - MM4
    
% Calculate Liquid fracion derivatives
% dX_i/dt = L_i+1 * (X_i+1 - X_i)  +  L_fi * (X_fi - X_i)  -  V_i * Y_i  +
%           V_i-1 * Y_i-1  +  (Q_i * X_i)/lambda
    ddt_X = zeros(p.N, 1);
    ddt_X(1) = (v.L(2,:).*(X(2,:) - X(1,:)) - v.V(1,:).*v.Y(1,:) + v.V0.*v.Y0)./MM(1,:);
    for n = 2:p.N-1
        ddt_X(n) = (v.L(n+1,:)*(X(n+1,:) - X(n,:)) - v.V(n,:)*v.Y(n,:)...
                   + v.V(n-1,:)*v.Y(n-1,:))./MM(n,:);
    end
    ddt_X(2) = ddt_X(2) + (u.LF(t).*(u.XF(t) - X(2,:)))./MM(2,:);
    ddt_X(p.N) = (v.LR*(X(end,:) - X(p.N,:)) - v.V(p.N,:)*v.Y(p.N,:)...
                 + v.V(p.N-1,:)*v.Y(p.N-1,:))./MM(p.N,:);
