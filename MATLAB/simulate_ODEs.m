...
% Calculate Liquid fracion derivatives
    ddt_X = zeros(p.N, 1);
    ddt_X(1) = (v.L(2).*(MM(2+p.N)) + v.L(1).*(1-MM(1+p.N)) - v.V(1).*v.Y(1) + v.V0.*v.Y0 + u.Q{n}(t).*MM(1+p.N))./MM(1);
    for n = 2:p.N-1
        ddt_X(n) = (v.L(n+1)*(MM(n+1+p.N)) + v.L(n)*(1-MM(n+p.N)) - v.V(n)*v.Y(n) + v.V(n-1)*v.Y(n-1) + u.Q{n}(t).*MM(n+p.N))./MM(n);
    end
    ddt_X(p.N) = (v.LR*(MM(p.N*2+2)) + v.L(p.N)*(1-MM(p.N*2)) - v.V(p.N)*v.Y(p.N) + v.V(p.N-1)*v.Y(p.N-1) + u.Q{p.N}(t).*MM(p.N*2))./MM(p.N);
